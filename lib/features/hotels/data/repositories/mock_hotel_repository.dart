...
class MockHotelRepository implements HotelRepository {
  ...
  Stream<List<Province>> watchProvinces() =>
      Stream.value(MockHotelDataSource.provinces);
  @override
  Stream<List<Hotel>> watchHotels() async* {
    yield _hotels;
  ...
  @override
  Future<List<Hotel>> getSimilarHotels(String hotelId) async {
    final hotel = await getHotelById(hotelId);
    if (hotel == null) return const [];
    return _hotels
        .where((item) =>
            item.id != hotelId && item.province.id == hotel.province.id)
        .take(5)
        .toList(growable: false);
  }

  @override
  Future<void> setFavorite(String hotelId, bool isFavorite) async {
    _hotels = _hotels
        .map((hotel) => hotel.id == hotelId
            ? hotel.copyWith(isFavorite: isFavorite)
            : hotel)
        .toList(growable: false);
    _controller.add(_hotels);
  }
}
